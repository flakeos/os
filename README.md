...
Set hostname and username in src/hosts/target-host/meta.nix. Run nixos-install flake hash target-host. Set a password and reboot.

For ISO generation run nix build hash packages.x86_64-linux.iso-minimal for headless or nix build hash packages.x86_64-linux.iso-graphical for desktop.
